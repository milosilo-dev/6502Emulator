use crate::bus::Bus;
use crate::platform::logging::log;
        ...
        while min_ticks > ticks{
            ...
            match ins{
                0xA9 => {
                    ...
                    log(format!("Loaded {:X} into the A register!", self.a));
                }
                0xA5 => {
                    // LDA_ZP
                ...
                0xA2 => {
                    ...
                    log(format!("Loaded {:X} into the X register!", self.x));
                }
                0xA6 => {
                    // LDX_ZP
                ...
                0xA0 => {
                    ...
                    log(format!("Loaded {:X} into the Y register!", self.y));
                }
                0xA4 => {
                    // LDY_ZP
                ...
                0x4C => {
                    ...
                    log(format!("Jumped to {:X}", self.pc));
                }
                0x6C => {
                    // INDIRECT_JMP
                ...
                0x20 => {
                    ...
                    log(format!("JSR to {:X}", target));
                }
                0x69 => {
                    // ADC_IMMEDIATE
                    let value = self.immediate_adressing(bus, &mut ticks);
                    self.adc(value);
                    log(format!("Got {:X} from adc", self.a));
                }
                0x65 => {
                    // ADC_ZP
                ...
                0x29 => {
                    ...
                    let value = self.immediate_adressing(bus, &mut ticks);
                    self.and(value);
                    log(format!("Got {:X} from and", self.a));
                }
                0x25 => {
                    // AND_ZP
                    let value = self.zero_page_adressing(bus, &mut ticks);
                ...
                0x0A => {
                    ...
                    self.asl_acc(value);
                    log(format!("Got {:X} from asl on {:X}", self.a, value));
                }
                0x06 => {
                    // ASL_ZP
                    let addr = self.get_zp_adress(bus, &mut ticks);
                    self.asl_mem(bus, addr, &mut ticks);
                    log(format!("Got {:X} from asl at addr {:X}", self.a, addr));
                }
                0x16 => {
                    // ASL_ZP_X
                ...
                0x24 => {
                    ...
                    log(format!("Bit test result: Z: {}, V: {}, N: {}", 
                        (self.status >> 1) & 1 == 1,
                        (self.status >> 6) & 1 == 1,
                        (self.status >> 7) & 1 == 1));
                }
                ...
                0x00 => {
                    ...
                    log(format!("Interupt happend"));
                }
                0x18 => {
                    // CLEAR_CARRY_FLAG
                    self.set_status(false, 0);
                    log(format!("Carry cleared"));
                }
                0xD8 => {
                    // CLEAR_DECIMAL_MODE
                    self.set_status(false, 3);
                    log(format!("Decimal mode cleared"));
                }
                0x58 => {
                    // CLEAR_INTERUPT_DISABLE
                    self.set_status(false, 2);
                    log(format!("Interupt disable cleared"));
                }
                0xB8 => {
                    // CLEAR_OVERFLOW_FLAG
                    self.set_status(false, 6);
                    log(format!("Overflow cleared"));
                }
                0xC9 => {
                    // COMPARE_IMMEDIATE
                    let value = self.immediate_adressing(bus, &mut ticks);
                    ...
                    ticks += 1;
                    log(format!("Compared acc and {:X}, C: {:X}, Z: {:X}, N: {:X}",
                        value,
                ...
                0xE0 => {
                    ...
                    ticks += 1;
                    log(format!("Compared X and {:X}, C: {:X}, Z: {:X}, N: {:X}",
                        value,
                        self.status & 0b00000001,
                        self.status & 0b00000010,
                        self.status & 0b10000000));
                ...
                0xC6 => {
                    ...
                    log(format!("Decremented addr: {:X}", addr));
                }
                0xD6 => {
                    // DEC_ZP_X
                    let addr = self.get_zp_adress_x(bus, &mut ticks);
                ...
                0xCA => {
                    // DEC_X
                    self.decx(&mut ticks);
                    log(format!("Decremented the x register to {:X}", self.x));
                }
                0x88 => {
                    // DEC_Y
                    self.decy(&mut ticks);
                    log(format!("Decremented the y register to {:X}", self.y));
                }
                0x49 => {
                    // EOR_IMMEDIATE
                    let value = self.immediate_adressing(bus, &mut ticks);
                    self.eor(&mut ticks, value);
                    log(format!("Exlusive ORe'd {:X} with {:X}", self.a, value));
                }
                0x45 => {
                    // EOR_ZERO_PAGE
                    let value = self.zero_page_adressing(bus, &mut ticks);
                ...
                0xE6 => {
                    ...
                    log(format!("Increment addr: {:X}", addr));
                }
                0xF6 => {
                    // INC_ZP_X
                    let addr = self.get_zp_adress_x(bus, &mut ticks);
                ...
                0xE8 => {
                    // INC_X
                    self.incx(&mut ticks);
                    log(format!("Increment the x register to {:X}", self.x));
                }
                0xC8 => {
                    // INC_Y
                    self.incy(&mut ticks);
                    log(format!("Increment the y register to {:X}", self.x));
                }
                0x09 => {
                    // ORA_IMMEDIATE
                    let value = self.immediate_adressing(bus, &mut ticks);
                    self.ora(value, &mut ticks);
                    log(format!("Logical Inclusive OR on {:X}", self.x));
                }
                0x05 => {
                    // ORA_ZERO_PAGE
                    let value = self.zero_page_adressing(bus, &mut ticks);
                ...
                0x48 => {
                    ...
                    log(format!("Pushed the contense of the accumulator to the stack"));
                }
                0x08 => {
                    // PHP
                    self.php(bus, &mut ticks);
                    log(format!("Pushed the processor status to the stack"));
                }
                0x68 => {
                    // PLA
                    self.pla(bus, &mut ticks);
                    log(format!("Got {:X} from the stack", self.a));
                }
                0x28 => {
                    // PLP
                    self.plp(bus, &mut ticks);
                    log(format!("Got {:X} from the stack", self.status));
                }
                0x2A => {
                    // ROL_ACC
                    self.rol_a(&mut ticks);
                    log(format!("Rotated the acc left"));
                }
                0x26 => {
                    // ROL_ZP
                    let addr = self.get_zp_adress(bus, &mut ticks);
                    self.rol(bus, addr as u16, &mut ticks);
                    log(format!("Rotated the {:X} left", addr));
                }
                0x36 => {
                    // ROL_ZP_X
                    let addr = self.get_zp_adress_x(bus, &mut ticks);
                ...
                0x6A => {
                    ...
                    log(format!("Rotated the acc right"));
                }
                0x66 => {
                    // ROR_ZP
                    let addr = self.get_zp_adress(bus, &mut ticks);
                    self.ror(bus, addr as u16, &mut ticks);
                    log(format!("Rotated the {:X} right", addr));
                }
                0x76 => {
                    // ROR_ZP_X
                    let addr = self.get_zp_adress_x(bus, &mut ticks);
                ...
                0x40 => {
                    ...
                    log(format!("Returned from Interrupt"));
                }
                0x60 => {
                    // RTS
                    self.rts(bus, &mut ticks);
                    log(format!("Returned from subroutine"));
                }
                0x38 => {
                    // SEC
                    self.set_status(true, 0);
                    ticks += 1;
                    log(format!("Set carry flag"));
                }
                0xF8 => {
                    // SED
                    self.set_status(true, 3);
                    ticks += 1;
                    log(format!("Set decimal flag"));
                }
                0x78 => {
                    // SEI
                    self.set_status(true, 2);
                    ticks += 1;
                    log(format!("Set interrupt disable"));
                }
                0xE9 => {
                    // SBC_IMMD
                    let value = self.immediate_adressing(bus, &mut ticks);
                    self.sbc(value);
                    log(format!("Subtracted {:X} from the acc", value));
                }
                0xE5 => {
                    // SBC_ZP
                    let value = self.zero_page_adressing(bus, &mut ticks);
                ...
                0x85 => {
                    ...
                    log(format!("Stored Acc at {:X}", addr));
                }
                0x95 => {
                    // STA_ZERO_PAGE_X
                    let addr = self.get_zp_adress_x(bus, &mut ticks);
                ...
                0xAA => {
                    // TAX
                    self.tax(&mut ticks);
                    log(format!("Tranfered A into X regiser"));
                }
                0xA8 => {
                    // TAY
                    self.tay(&mut ticks);
                    log(format!("Tranfered A into Y regiser"));
                }
                0xBA => {
                    // TSX
                    self.tsx(&mut ticks);
                    log(format!("Tranfered SP into X regiser"));
                }
                0x8A => {
                    // TXA
                    self.txa(&mut ticks);
                    log(format!("Tranfered X into A regiser"));
                }
                0x9A => {
                    // TXS
                    self.txs(&mut ticks);
                    log(format!("Tranfered X into S regiser"));
                }
                0x98 => {
                    // TYA
                    self.tya(&mut ticks);
                    log(format!("Tranfered Y into A regiser"));
                }
                0xEA => {
                    // NOP
                    ticks += 1;
                ...
                _ => {
                    log(format!("Tried to execute unknown command!"));
                }
            }
        }
    }
